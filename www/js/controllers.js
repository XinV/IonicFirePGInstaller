    ...
    bucketListRef.on('value', function(snapshot) {
        ...
        if(data) {
          ...
          if(data.temperature) {
            $scope.list.push(
              {
                "item": "Temperature:   " + data["temperature"] + " F",
                "key": "temperature"
              }
            );
          }
          ...
          if(data.noise) {
            ...
            if(noise >= 130) {
              noiseLevel = " (Jet Take-off)"
            } else if(noise >= 110) {
              noiseLevel = " (Rock Concert)"
            } else if (noise >= 90) {
              noiseLevel = " (Construction Site)"
            }else if (noise >= 80) {
              noiseLevel = " (Noisy Workspace)"
            }else if (noise >= 70) {
              noiseLevel = " (Average Street Traffic)"
            }else if (noise >= 60) {
              noiseLevel = " (Busy Office)"
            }else if (noise >= 40) {
              noiseLevel = " (Living Room)"
            }else if (noise >= 30) {
              noiseLevel = " (Library)"
            }else if (noise >= 20) {
              noiseLevel = " (Bedroom)"
            }else if (noise >= 0) {
              noiseLevel = " (Deep Night)"
            }

            $scope.list.push(
              {
                "item": "Noise Level:   " + data["noise"] + " dB " + noiseLevel,
                "key": "noise"
              }
            );
          }
